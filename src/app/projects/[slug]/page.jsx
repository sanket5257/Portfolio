...
export default function ProjectDetails(promiseParams) {
  ...
  const section1ImgRef = useRef(null);
    ...
    <div className="min-h-screen bg-[#D9D9D9] w-full overflow-hidden">
      ...
      <div id="section3" className="py-[20vw] w-full flex justify-center font-title items-center">
        <div className="w-[90vw] md:w-[70vw] flex flex-col md:flex-row items-start gap-10 text-center md:text-start">
          <h1 ref={titleRef} className="md:w-[30%] text-[5vw] md:text-[1.5vw] font-extrabold">
            About
          </h1>
          <div ref={paraRef} className="md:w-[70%] space-y-4 text-[#666666] text-sm md:text-base">
      ...
      <div className="relative px-4 sm:px-0">
        ...
        <div className="h-[40vw] sm:h-[18vw] w-[40vw] sm:w-[18vw] rounded-full bg-[#1D1D1D] flex justify-center items-center absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 transition-transform duration-300 hover:scale-120 cursor-pointer">
          <div className="flex flex-col sm:flex-row justify-center items-center gap-2 text-center">
            <img className="size-8 sm:size-8" src="/arrowup.png" alt="See live" />
      ...
      <div id="desktop-view" className="h-screen mb-[20vh] mt-[10vh] w-full flex relative flex-col justify-center items-center">
        <img ref={imgLeftRef} src={project.overviewimg} className="h-[30vw] pl-[20vw]" alt="" />
        <img ref={imgRightRef} src={project.overviewimg} className="h-[30vw] absolute -bottom-40 pr-[20vw]" alt="" />
      </div>

      {/* Next Project Link */}
      <div id="projects" className="relative overflow-hidden pt-20 cursor-none">
        {projects
          .filter((p) => p.slug !== project.slug) // show only other projects
          .map((next) => (
            <Link key={next.slug} href={`/projects/${next.slug}`} className="relative">
              <img src={next.image} className="md:h-full h-screen md:w-full project-image object-cover" alt={next.title} />
              <div className="absolute top-0 left-0 w-full h-full bg-black/10 flex flex-col justify-center items-center text-[#D9D9D9] leading-[15vw] sm:leading-[9.5vw]">
                <h1>Next Project</h1>
                <h1 className="font-title font-extrabold uppercase text-[14vw] sm:text-[10vw]">{next.heading1}</h1>
                <h1 className="font-title2 text-[14vw] sm:text-[10vw]">{next.heading2}</h1>
              </div>
            </Link>
          ))}
      </div>
